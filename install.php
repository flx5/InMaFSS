...
echo '<title>InMaFSS - Installer // Step '.$step.'</title>';
echo '</head><body>';
...
function getPage() {
    ...
    switch ($step) {
        ...
        case 3:
            ...
            echo '<tr><td>' . $loc['dbtype'] . ':</td><td>';
            echo '<select name="dbtype">';
            if ($fh = opendir("./inc/db/")) {
                while (($file = readdir($fh)) !== false) {
                    if (!is_dir($file)) {
        ...
        case 4:
            if (!isset($_POST['dbtype']) || !isset($_POST['host']) || !isset($_POST['username']) || !isset($_POST['password']) || !isset($_POST['database'])) {
            ...
            $db = SQL::GenerateInstance($_POST['dbtype'], $_POST['host'], $_POST['username'], $_POST['password'], $_POST['database']);
            ...
            $text = getConfig($_POST['dbtype'], $_POST['host'], $_POST['username'], $_POST['password'], $_POST['database'], $lang);

            echo '<br>' . $loc['name.of.school'] . ':&nbsp;&nbsp;&nbsp;<input type="text" value="" name="schoolname"><br><hr>';
            ...
            if (!is_writable("./inc/")) {
                ...
                file_put_contents("./inc/config.php", $text);
        ...
        case 5:
            global $lang;
            if (!isset($_POST['schoolname']) || !file_exists("./inc/config.php")) {
                header("Location: install.php?step=" . ($step - 1) . '&lang=' . $lang);
                exit;
            }
            ...
            $db = SQL::GenerateInstance($dbtype, $dbhost, $dbusr, $dbpass, $dbname);
            ...
            $db->DoQuery("INSERT INTO pages (order_num, title, content, timestamp_from, timestamp_end) VALUES ('1', '" . $loc['welcome'] . "', '" . $loc['firstpage'] . "', '" . time() . "', '" . $tomorrow . "')");
            
            $schoolname = $_POST['schoolname']; 
            $schoolname = htmlentities($schoolname, ENT_SUBSTITUTE, "UTF-8"); 
        ...
        case 7:
            ...
            $db->DoQuery("INSERT INTO users (username, password) VALUES ('" . $_POST['username'] . "', '" . $pw . "')");
...
function getSql() {
  ...
  `system` enum('willi2') NOT NULL DEFAULT 'willi2',
...
function getConfig($dbtype, $dbhost, $usr, $passwd, $db) {
